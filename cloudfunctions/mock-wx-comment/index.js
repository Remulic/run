...
const COMMENT = 'cloud_wx_comment';
const PAGE_SIZE = 20
...
exports.main = async (event, context) => {
  ...
  switch (action) {
    case 'saveOne': {
      ...
      let execRes = await db.collection(COMMENT).add({
        data: params
      })
    ...
    case 'deleteByPostId':{
      ...
      let execRes = await db.collection(COMMENT).where({
        postId,
        openid
      }).remove();
    ...
    case 'getList': {
      ...
      let execRes = await db.collection(COMMENT).where(queryParams).skip(P_SIZE * pageNum).orderBy('created_time', 'asc').limit(PAGE_SIZE).get();
      /**
       * 封包结果
       */
    ...
    case 'deleteOne': {
       ...
       * 调用云函数清理对应的点赞评论信息
       */
      let execRes = await db.collection(COMMENT).doc(docId).remove();
      return ResponseDTO.succMsg("删除成功!");
      break
    }
